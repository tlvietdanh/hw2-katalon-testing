...
switch(var_statusExpectedResult.toString())
{
	case 'password is empty':
		'Verify when expected test case signup with password is empty'
		def errorMess = WebUI.getText(findTestObject('Object Repository/Page_Mattermost_Signup/label_status_password'))
		WebUI.verifyEqual(errorMess.contains('This field is required'), true)
		break;
	case 'password is not correct':
		'Verify when expected test case signup fail with password is not correct'
		def errorMess = WebUI.getText(findTestObject('Object Repository/Page_Mattermost_Signup/label_status_password'))
		WebUI.verifyEqual(errorMess.contains('Your password must contain between 5 and 64 characters.'), true)
		break;
}
