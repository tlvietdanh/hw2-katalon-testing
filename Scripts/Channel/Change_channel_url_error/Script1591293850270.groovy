...
if(WebUI.verifyElementPresent(findTestObject('Object Repository/Rename_channel/span_Town Square'), 1, FailureHandling.OPTIONAL)) {
	...
	if(WebUI.verifyElementPresent(findTestObject('Object Repository/Rename_channel/p_URLs must have at least 2 characters'), 1, FailureHandling.OPTIONAL)) {
		def error = WebUI.getText(findTestObject('Object Repository/Rename_channel/p_URLs must have at least 2 characters'))
		if(newChannelUrl.length() < 2) {
			WebUI.verifyEqual(error.contains('URLs must have at least 2 characters'), true)
		}
		else {
			WebUI.verifyEqual(error.contains('A channel with that handle already exists'), true)
		}
	}		
}
